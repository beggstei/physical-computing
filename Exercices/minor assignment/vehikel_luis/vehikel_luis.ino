...
void loop() {
  ...
  //liest die beiden helligkeitssensoren aus
  brightnessLeft = analogRead(A0);
  brightnessRight = analogRead(A1);

  //berechnet die Helligekeit der birne
  ...
  //schickt werte an computer, zur überwachung
  Serial.println(brightnessLeft);
  Serial.println(brightnessRight);
  Serial.println(measureDistance());

  //wenn die distanz zum nächsten hindernis grösser als 30 cm ist, folge dem licht
  if(measureDistance() > 30){
    ...
    // sonst fahre nach hinten links zurück
  }else{
    driveBackward();
    turnLeftBackward();
  }
...
void driveForward(){
  analogWrite(SPEED_PIN1, 150);
  analogWrite(SPEED_PIN2, 150);
