...
void setup() {
  ...
  client.begin("b3n14m1negg5te1n.cloud.shiftr.io", net);
...
void loop() {
  client.loop();
  delay(10);
  diameterNew = map(analogRead(A0), 0, 1023, 10, 300);
  ...
  if (diameterOld != diameterNew){
      client.publish("/diameter", "diameter," + String(diameterNew));
      diameterOld = diameterNew;
    }
...
void connect() {
  ...
  while (!client.connect("arduino", "b3n14m1negg5te1n", "YlDpbrYsvqBZNgaR")) {
    Serial.print(".");
    delay(1000);
  }
